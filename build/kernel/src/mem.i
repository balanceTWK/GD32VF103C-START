...
void rt_free(void *rmem)
{
    ...
    do { rt_base_t level; level = rt_hw_interrupt_disable(); if (rt_interrupt_get_nest() != 0) { rt_kprintf("Function[%s] shall not be used in ISR\n", __FUNCTION__); if (!(0)) { rt_assert_handler("0", __FUNCTION__, 553); } } rt_hw_interrupt_enable(level); } while (0);

    if (!((((rt_ubase_t)rmem) & (4 - 1)) == 0)) { rt_assert_handler("(((rt_ubase_t)rmem) & (RT_ALIGN_SIZE - 1)) == 0", __FUNCTION__, 555); };
...
# 556 "rt-thread/src/mem.c"
    ...
    if (!(mem->used)) { rt_assert_handler("mem->used", __FUNCTION__, 587); };
    if (!(mem->magic == 0x1ea0)) { rt_assert_handler("mem->magic == HEAP_MAGIC", __FUNCTION__, 588); };
