...
# 34 "rt-thread/src/scheduler.c" 2
...
# 35 "rt-thread/src/scheduler.c" 2
...
{
    if (!(thread != (0))) { rt_assert_handler("thread != RT_NULL", __FUNCTION__, 83); };
...
void rt_schedule(void)
{
    ...
    if (rt_scheduler_lock_nest == 0)
    {
        ...
        if (rt_thread_ready_priority_group != 0)
        {
            ...
            if ((rt_current_thread->stat & 0x0f) == 0x03)
            {
            ...
            if (to_thread != rt_current_thread)
            {
                ...
                rt_schedule_remove_thread(to_thread);
                to_thread->stat = 0x03 | (to_thread->stat & ~0x0f);
...
# 498 "rt-thread/src/scheduler.c"
            ...
            else
            {
                ...
                rt_current_thread->stat = 0x03 | (rt_current_thread->stat & ~0x0f);
            }
        }
    }
...
# 688 "rt-thread/src/scheduler.c"
void rt_schedule_insert_thread(struct rt_thread *thread)
{
    register rt_base_t temp;

    if (!(thread != (0))) { rt_assert_handler("thread != RT_NULL", __FUNCTION__, 692); };
    ...
    {
        thread->stat = 0x03 | (thread->stat & ~0x0f);
        goto __exit;
    }
...
# 785 "rt-thread/src/scheduler.c"
void rt_schedule_remove_thread(struct rt_thread *thread)
{
    register rt_base_t level;

    if (!(thread != (0))) { rt_assert_handler("thread != RT_NULL", __FUNCTION__, 789); };
