...
rt_err_t rt_mp_detach(struct rt_mempool *mp)
{
    ...
    if (!(mp != (0))) { rt_assert_handler("mp != RT_NULL", __FUNCTION__, 141); };
    if (!(rt_object_get_type(&mp->parent) == RT_Object_Class_MemPool)) { rt_assert_handler("rt_object_get_type(&mp->parent) == RT_Object_Class_MemPool", __FUNCTION__, 142); };
    if (!(rt_object_is_systemobject(&mp->parent))) { rt_assert_handler("rt_object_is_systemobject(&mp->parent)", __FUNCTION__, 143); };
                     ...
                     rt_size_t block_size)
{
    ...
    do { rt_base_t level; level = rt_hw_interrupt_disable(); if (rt_interrupt_get_nest() != 0) { rt_kprintf("Function[%s] shall not be used in ISR\n", __FUNCTION__); if (!(0)) { rt_assert_handler("0", __FUNCTION__, 196); } } rt_hw_interrupt_enable(level); } while (0);


    if (!(name != (0))) { rt_assert_handler("name != RT_NULL", __FUNCTION__, 199); };
    if (!(block_count > 0 && block_size > 0)) { rt_assert_handler("block_count > 0 && block_size > 0", __FUNCTION__, 200); };
...
;
# 255 "rt-thread/src/mempool.c"
rt_err_t rt_mp_delete(rt_mp_t mp)
{
    struct rt_thread *thread;
    register rt_ubase_t temp;

    do { rt_base_t level; level = rt_hw_interrupt_disable(); if (rt_interrupt_get_nest() != 0) { rt_kprintf("Function[%s] shall not be used in ISR\n", __FUNCTION__); if (!(0)) { rt_assert_handler("0", __FUNCTION__, 260); } } rt_hw_interrupt_enable(level); } while (0);


    if (!(mp != (0))) { rt_assert_handler("mp != RT_NULL", __FUNCTION__, 263); };
    if (!(rt_object_get_type(&mp->parent) == RT_Object_Class_MemPool)) { rt_assert_handler("rt_object_get_type(&mp->parent) == RT_Object_Class_MemPool", __FUNCTION__, 264); };
    if (!(rt_object_is_systemobject(&mp->parent) == 0)) { rt_assert_handler("rt_object_is_systemobject(&mp->parent) == RT_FALSE", __FUNCTION__, 265); };
...
void *rt_mp_alloc(rt_mp_t mp, rt_int32_t time)
{
    ...
    if (!(mp != (0))) { rt_assert_handler("mp != RT_NULL", __FUNCTION__, 319); };
    ...
    while (mp->block_free_count == 0)
    {
        ...
        do { rt_base_t level; level = rt_hw_interrupt_disable(); if (rt_interrupt_get_nest() != 0) { rt_kprintf("Function[%s] shall not be used in ISR\n", __FUNCTION__); if (!(0)) { rt_assert_handler("0", __FUNCTION__, 340); } } rt_hw_interrupt_enable(level); } while (0);
    ...
    if (!(block_ptr != (0))) { rt_assert_handler("block_ptr != RT_NULL", __FUNCTION__, 385); };
