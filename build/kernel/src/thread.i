                                ...
                                rt_uint32_t tick)
{
    ...
    if (!(priority < 32)) { rt_assert_handler("priority < RT_THREAD_PRIORITY_MAX", __FUNCTION__, 149); };
                        ...
                        rt_uint32_t tick)
{
    ...
    if (!(thread != (0))) { rt_assert_handler("thread != RT_NULL", __FUNCTION__, 241); };
    if (!(stack_start != (0))) { rt_assert_handler("stack_start != RT_NULL", __FUNCTION__, 242); };
...
rt_err_t rt_thread_startup(rt_thread_t thread)
{
    ...
    if (!(thread != (0))) { rt_assert_handler("thread != RT_NULL", __FUNCTION__, 291); };
    if (!((thread->stat & 0x0f) == 0x00)) { rt_assert_handler("(thread->stat & RT_THREAD_STAT_MASK) == RT_THREAD_INIT", __FUNCTION__, 292); };
    if (!(rt_object_get_type((rt_object_t)thread) == RT_Object_Class_Thread)) { rt_assert_handler("rt_object_get_type((rt_object_t)thread) == RT_Object_Class_Thread", __FUNCTION__, 293); };
...
rt_err_t rt_thread_detach(rt_thread_t thread)
{
    ...
    if (!(thread != (0))) { rt_assert_handler("thread != RT_NULL", __FUNCTION__, 336); };
    if (!(rt_object_get_type((rt_object_t)thread) == RT_Object_Class_Thread)) { rt_assert_handler("rt_object_get_type((rt_object_t)thread) == RT_Object_Class_Thread", __FUNCTION__, 337); };
    if (!(rt_object_is_systemobject((rt_object_t)thread))) { rt_assert_handler("rt_object_is_systemobject((rt_object_t)thread)", __FUNCTION__, 338); };
...
rt_err_t rt_thread_delete(rt_thread_t thread)
{
    ...
    if (!(thread != (0))) { rt_assert_handler("thread != RT_NULL", __FUNCTION__, 435); };
    if (!(rt_object_get_type((rt_object_t)thread) == RT_Object_Class_Thread)) { rt_assert_handler("rt_object_get_type((rt_object_t)thread) == RT_Object_Class_Thread", __FUNCTION__, 436); };
    if (!(rt_object_is_systemobject((rt_object_t)thread) == 0)) { rt_assert_handler("rt_object_is_systemobject((rt_object_t)thread) == RT_FALSE", __FUNCTION__, 437); };
...
rt_err_t rt_thread_sleep(rt_tick_t tick)
{
    ...
    thread = rt_thread_self();
    if (!(thread != (0))) { rt_assert_handler("thread != RT_NULL", __FUNCTION__, 494); };
    if (!(rt_object_get_type((rt_object_t)thread) == RT_Object_Class_Thread)) { rt_assert_handler("rt_object_get_type((rt_object_t)thread) == RT_Object_Class_Thread", __FUNCTION__, 495); };
...
rt_err_t rt_thread_control(rt_thread_t thread, int cmd, void *arg)
{
    ...
    if (!(thread != (0))) { rt_assert_handler("thread != RT_NULL", __FUNCTION__, 567); };
    if (!(rt_object_get_type((rt_object_t)thread) == RT_Object_Class_Thread)) { rt_assert_handler("rt_object_get_type((rt_object_t)thread) == RT_Object_Class_Thread", __FUNCTION__, 568); };
...
rt_err_t rt_thread_suspend(rt_thread_t thread)
{
    ...
    if (!(thread != (0))) { rt_assert_handler("thread != RT_NULL", __FUNCTION__, 663); };
    if (!(rt_object_get_type((rt_object_t)thread) == RT_Object_Class_Thread)) { rt_assert_handler("rt_object_get_type((rt_object_t)thread) == RT_Object_Class_Thread", __FUNCTION__, 664); };
    ...
    if (stat == 0x03)
    {
        ...
        if (!(thread == rt_thread_self())) { rt_assert_handler("thread == rt_thread_self()", __FUNCTION__, 681); };
    }
...
;
# 706 "rt-thread/src/thread.c"
rt_err_t rt_thread_resume(rt_thread_t thread)
{
    register rt_base_t temp;


    if (!(thread != (0))) { rt_assert_handler("thread != RT_NULL", __FUNCTION__, 711); };
    if (!(rt_object_get_type((rt_object_t)thread) == RT_Object_Class_Thread)) { rt_assert_handler("rt_object_get_type((rt_object_t)thread) == RT_Object_Class_Thread", __FUNCTION__, 712); };
...
void rt_thread_timeout(void *parameter)
{
    ...
    if (!(thread != (0))) { rt_assert_handler("thread != RT_NULL", __FUNCTION__, 756); };
    if (!((thread->stat & 0x0f) == 0x02)) { rt_assert_handler("(thread->stat & RT_THREAD_STAT_MASK) == RT_THREAD_SUSPEND", __FUNCTION__, 757); };
    if (!(rt_object_get_type((rt_object_t)thread) == RT_Object_Class_Thread)) { rt_assert_handler("rt_object_get_type((rt_object_t)thread) == RT_Object_Class_Thread", __FUNCTION__, 758); };
...
rt_thread_t rt_thread_find(char *name)
{
    ...
    if (!(information != (0))) { rt_assert_handler("information != RT_NULL", __FUNCTION__, 795); };
