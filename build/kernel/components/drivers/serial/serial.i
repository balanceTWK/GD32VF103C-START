...
# 1 "rt-thread/components/drivers/serial/serial.c"
# 27 "rt-thread/components/drivers/serial/serial.c"
...
# 28 "rt-thread/components/drivers/serial/serial.c" 2
...
# 30 "rt-thread/components/drivers/serial/serial.c" 2

#define DBG_TAG "UART"
#define DBG_LVL DBG_INFO
# 1 "rt-thread/include/rtdbg.h" 1
# 31 "rt-thread/include/rtdbg.h"
#define RT_DBG_H__ 
...
# 140 "rt-thread/include/rtdbg.h"
#define dbg_raw(...) rt_kprintf(__VA_ARGS__);
# 154 "rt-thread/include/rtdbg.h"
#define LOG_D(...) 
...
# 206 "rt-thread/components/drivers/serial/serial.c"
static __inline int _serial_poll_rx(struct rt_serial_device *serial, rt_uint8_t *data, int length)
{
    int ch;
    int size;

    if (!(serial != (0))) { rt_assert_handler("serial != RT_NULL", __FUNCTION__, 211); };
    size = length;
...
{
    int size;
    if (!(serial != (0))) { rt_assert_handler("serial != RT_NULL", __FUNCTION__, 231); };
...
static __inline int _serial_int_rx(struct rt_serial_device *serial, rt_uint8_t *data, int length)
{
    ...
    if (!(rx_fifo != (0))) { rt_assert_handler("rx_fifo != RT_NULL", __FUNCTION__, 266); };
...
static __inline int _serial_int_tx(struct rt_serial_device *serial, const rt_uint8_t *data, int length)
{
    ...
    if (!(serial != (0))) { rt_assert_handler("serial != RT_NULL", __FUNCTION__, 310); };

    size = length;
    tx = (struct rt_serial_tx_fifo*) serial->serial_tx;
    if (!(tx != (0))) { rt_assert_handler("tx != RT_NULL", __FUNCTION__, 314); };
...
static rt_err_t rt_serial_init(struct rt_device *dev)
{
    ...
    if (!(dev != (0))) { rt_assert_handler("dev != RT_NULL", __FUNCTION__, 541); };
...
static rt_err_t rt_serial_open(struct rt_device *dev, rt_uint16_t oflag)
{
    ...
    if (!(dev != (0))) { rt_assert_handler("dev != RT_NULL", __FUNCTION__, 560); };
    serial = (struct rt_serial_device *)dev;
    ...
    if (serial->serial_rx == (0))
    {
        if (oflag & 0x100)
        {
            ...
            rx_fifo = (struct rt_serial_rx_fifo*) rt_malloc (sizeof(struct rt_serial_rx_fifo) +
                serial->config.bufsz);
            if (!(rx_fifo != (0))) { rt_assert_handler("rx_fifo != RT_NULL", __FUNCTION__, 591); };
...
# 632 "rt-thread/components/drivers/serial/serial.c"
        else
        {
            serial->serial_rx = (0);
        }
    ...
    if (serial->serial_tx == (0))
    {
        if (oflag & 0x400)
        {
            ...
            if (!(tx_fifo != (0))) { rt_assert_handler("tx_fifo != RT_NULL", __FUNCTION__, 654); };
...
static rt_err_t rt_serial_close(struct rt_device *dev)
{
    ...
    if (!(dev != (0))) { rt_assert_handler("dev != RT_NULL", __FUNCTION__, 705); };
    serial = (struct rt_serial_device *)dev;
    ...
    if (dev->open_flag & 0x100)
    {
        ...
        rx_fifo = (struct rt_serial_rx_fifo*)serial->serial_rx;
        if (!(rx_fifo != (0))) { rt_assert_handler("rx_fifo != RT_NULL", __FUNCTION__, 716); };
...
# 749 "rt-thread/components/drivers/serial/serial.c"
    if (dev->open_flag & 0x400)
    {
        ...
        if (!(tx_fifo != (0))) { rt_assert_handler("tx_fifo != RT_NULL", __FUNCTION__, 754); };
...
# 775 "rt-thread/components/drivers/serial/serial.c"
    return 0;
}
                                ...
                                rt_size_t size)
{
    ...
    if (!(dev != (0))) { rt_assert_handler("dev != RT_NULL", __FUNCTION__, 785); };
    if (size == 0) return 0;
                                 ...
                                 rt_size_t size)
{
    ...
    if (!(dev != (0))) { rt_assert_handler("dev != RT_NULL", __FUNCTION__, 811); };
    if (size == 0) return 0;
                                  ...
                                  void *args)
{
    ...
    if (!(dev != (0))) { rt_assert_handler("dev != RT_NULL", __FUNCTION__, 936); };
    serial = (struct rt_serial_device *)dev;
    ...
    switch (cmd)
    {
        ...
        case 0x03:
            ...
            break;
# 1077 "rt-thread/components/drivers/serial/serial.c"
        default :
                               ...
                               void *data)
{
    ...
    if (!(serial != (0))) { rt_assert_handler("serial != RT_NULL", __FUNCTION__, 1108); };
...
void rt_hw_serial_isr(struct rt_serial_device *serial, int event)
{
    switch (event & 0xff)
    {
        case 0x01:
        {
            ...
            rx_fifo = (struct rt_serial_rx_fifo*)serial->serial_rx;
            if (!(rx_fifo != (0))) { rt_assert_handler("rx_fifo != RT_NULL", __FUNCTION__, 1152); };
...
# 1272 "rt-thread/components/drivers/serial/serial.c"
    }
}
