...
#if !defined(BSP_USING_UART0) && !defined(BSP_USING_UART1) && !defined(BSP_USING_UART2) && !defined(BSP_USING_UART3) && !defined(BSP_USING_UART4) && !defined(BSP_USING_UART5)
...
static void usart_isr(struct rt_serial_device *serial)
{
    ...
    if ((usart_interrupt_flag_get(usart->usart_base, USART_INT_FLAG_RBNE) != RESET) && (RESET != usart_flag_get(usart->usart_base, USART_FLAG_RBNE)))
    {
...
int rt_hw_usart_init(void)
{
    ...
    for (index = 0; index < obj_num; index++)
    {
        ...
        result = rt_hw_serial_register(&usart_config[index].serial,
                                       ...
                                       RT_DEVICE_FLAG_RDWR | RT_DEVICE_FLAG_INT_RX | RT_DEVICE_FLAG_INT_TX, &usart_config[index]);
